...
class CarnetAdressesApp:
    ...
    def _construire_zone_boutons(self):
        ...
        tk.Button(cadre_boutons, text="Agenda", width=10, command=self.ouvrir_agenda).pack(side="left", padx=5)
    ...
    def ouvrir_agenda(self):
        if len(self.carnet) == 0:
            messagebox.showwarning("Attention", "Ajoutez au moins un contact avant d'ouvrir l'agenda.")
            return
        FenetreAgenda(self.racine, self.carnet)
